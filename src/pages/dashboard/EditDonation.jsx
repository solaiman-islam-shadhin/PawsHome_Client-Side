...
const EditDonation = () => {
  ...
  useEffect(() => {
    if (campaign) {
      reset({
        ...
        lastDate: new Date(campaign.lastDate).toISOString().split('T')[0],
  ...
  const updateCampaignMutation = useMutation({
    mutationFn: ({ id, data }) => donationAPI.updateCampaign(id, data),
    onSuccess: () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      updateCampaignMutation.mutate({ id, data: campaignData });
    } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Edit Donation Campaign</h1>
        <p className="text-gray-600 dark:text-gray-300 mt-2">
          Update your campaign for {campaign?.petName}
        </p>
      </div>
